...
@require_hadoop
def test_read_parquet_execution(setup):
    hdfs = setup
    ...
    with hdfs.open(f"{TEST_DIR}/test_partitioned/file1.parquet", "wb", replication=1) as f:
        test_df.to_parquet(f, row_group_size=3)
    with hdfs.open(f"{TEST_DIR}/test_partitioned/file2.parquet", "wb", replication=1) as f:
