...
class Test(unittest.TestCase):
    def testArrowDtype(self):
        ...
        self.assertEqual(ArrowListDtype(ArrowListDtype(str)),
                         ArrowListDtype.construct_from_string('Arrow[List[Arrow[string]]]'))
    ...
    def testToPandas(self):
        ...
        s = ('s' + df2['b']).astype(object)
        expected = 's' + df['b']
        pd.testing.assert_series_equal(s, expected)

        s2 = df2['b'].str[:2].astype(object)
        expected = df['b'].str[:2]
        pd.testing.assert_series_equal(s2, expected)

        s3 = (df2['b'] < 'ss').astype(bool)
        expected = df['b'] < 'ss'
        pd.testing.assert_series_equal(s3, expected)
