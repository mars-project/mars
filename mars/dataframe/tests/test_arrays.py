...
def test_arrow_dtype():
    ...
    assert ArrowListDtype(ArrowListDtype('string')) == ArrowListDtype.construct_from_string('Arrow[List[string]]')
...
def test_arrow_string_array_functions():
    ...
    for pandas_only in [False, True]:
        with option_context({'dataframe.arrow_array.pandas_only': pandas_only}):
            ...
            assert list(arrow_array4.astype(ArrowStringDtype(), copy=False)) == list(string_array4.astype(pd.StringDtype(), copy=False))
            assert list(arrow_array4.astype(ArrowStringDtype(), copy=True)) == list(string_array4.astype(pd.StringDtype(), copy=True))
            ...
            # test memory_usage
            if pandas_only:
                assert arrow_array.memory_usage(deep=False) == pd.Series(string_array).memory_usage(index=False)
            else:
            ...
            assert list(has_na_arrow_array.value_counts(dropna=True)) == list(has_na_string_array.value_counts(dropna=True))
            ...
            assert list((arrow_array + has_na_arrow_array).fillna('ss')) == list((string_array + has_na_string_array).fillna('ss'))
...
def test_arrow_list_functions():
    ...
    for pandas_only in [False, True]:
        with option_context({'dataframe.arrow_array.pandas_only': pandas_only}):
            ...
            # test take
            assert list(arrow_array.take([1, 2, -1])) == list(pd.Series(lst).take([1, 2, -1]))
