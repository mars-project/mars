...
async def test_mars_with_xgboost_sklearn_reg(ray_large_cluster, create_cluster):
    ...
    with session:
        ...
        X, y = md.DataFrame(X), md.DataFrame({"target": y})
        df: md.DataFrame = md.concat([md.DataFrame(X), md.DataFrame(y)], axis=1)
        df.execute()

        num_shards = 4
        ds = md.to_ray_mldataset(df, num_shards)
        assert isinstance(ds, ml_dataset.MLDataset)
        ...
        clf = RayXGBRegressor(
            ray_params=RayParams(num_actors=2, cpus_per_actor=1), random_state=42
        )
        # train
        train_set = RayDMatrix(ds, "target")
        clf.fit(train_set, y=None)
        print(f"Regressor {clf}")
        pred = clf.predict(train_set)
        print("predicted values: ", pred)
