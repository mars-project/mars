...
def test_where_execution(setup):
    ...
    pd.testing.assert_frame_equal(new_df.execute().fetch(),
                                  raw_df.mask(raw_df < 0))
...
def test_set_axis_execution(setup):
    ...
    pd.testing.assert_frame_equal(df1.execute().fetch(),
                                  raw_df.set_axis(new_idx))
    ...
    s1 = s.copy()
    s1.index = new_idx
    pd.testing.assert_series_equal(s1.execute().fetch(),
                                   raw_series.set_axis(idx_data))
...
def test_sample_execution(setup):
    ...
    pd.testing.assert_frame_equal(r1.execute().fetch(),
                                  r2.execute().fetch())
    ...
    pd.testing.assert_series_equal(r1.execute().fetch(),
                                   r2.execute().fetch())
