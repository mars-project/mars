...
from ...serialize import AnyField, BoolField, DictField
...
_stage_info = namedtuple('_stage_info', ('map_groups', 'map_sources', 'combine_groups', 'combine_sources',
                                         'agg_sources', 'agg_columns', 'agg_funcs', 'valid_columns'))
_series_col_name = 'col_name'


def _mean_fun(sum_data, count_data, axis=0):
    return sum_data.sum(axis=axis) / count_data.sum(axis=axis)


def _reduce_var_fun(sum_data, count_data, var_data, axis=0):
...
def _reduce_std_fun(*args, **kwargs):
    return np.sqrt(_reduce_var_fun(*args, **kwargs))


_custom_funs = {
    'mean_fun': _mean_fun,
    'reduce_var_fun': _reduce_var_fun,
    'reduce_std_fun': _reduce_std_fun,
}
...
class DataFrameAggregate(DataFrameOperand, DataFrameOperandMixin):
    ...
    def __init__(self, func=None, axis=None, use_inf_as_na=None,map_groups=None, map_sources=None,
                 ...
                 agg_funcs=None, object_type=None, stage=None, **kw):
        super(DataFrameAggregate, self).__init__(
            _func=func, _axis=axis, _use_inf_as_na=use_inf_as_na,_map_groups=map_groups,
            _map_sources=map_sources, _combine_groups=combine_groups, _combine_sources=combine_sources,
            _agg_sources=agg_sources, _agg_columns=agg_columns, _agg_funcs=agg_funcs, _stage=stage,
    ...
    @classmethod
    def _gen_chunk_stage_info(cls, op_func, chunk_cols=None):
        ...
        def _add_column_to_functions(col, fun_name, mappers, combiners, aggregator):
            ...
            for mapper, combiner in zip(mappers, combiners):
                cls._safe_append(map_groups, mapper, col)
                cls._safe_append(combine_groups, (mapper, combiner), col)
                sources.append((mapper, combiner))

            for mapper, combiner in zip(mappers, combiners):
                if combiner in _custom_funs:
                    combine_sources[(mapper, combiner)] = sources

            agg_sources[fun_name] = sources
            cls._safe_append(agg_columns, fun_name, col)
            agg_funcs[fun_name] = aggregator
        ...
        for col, funcs in op_func.items():
            ...
            for func in funcs:
                if func in {'sum', 'prod', 'min', 'max'}:
                    ...
                    _add_column_to_functions(col, func, ['sum', 'count'], ['sum', 'sum'], 'mean_fun')
                elif func in {'var', 'std'}:
                    _add_column_to_functions(col, func, ['sum', 'count', 'var'],
                                             ['sum', 'sum', 'reduce_var_fun'],
                                             'reduce_var_fun' if func == 'var' else 'reduce_std_fun')
        ...
        return _stage_info(map_groups=_clean_dict(map_groups), map_sources=map_sources,
                           ...
                           agg_funcs=agg_funcs, valid_columns=valid_columns or None)

    @classmethod
    def _gen_map_chunks(cls, op, in_df, out_df, stage_infos, input_index_to_output):
        axis = op.axis
    ...
    def _execute_combine(cls, ctx, op):
        ...
        for fun_strs, df in zip(op.combine_groups.keys(), in_data):
            if fun_strs[-1] in _custom_funs:
                func = _custom_funs[fun_strs[-1]]
    ...
    def _execute_agg(cls, ctx, op):
        ...
        for func_name, func_sources in op.agg_sources.items():
            ...
            if func_str in _custom_funs:
                func = _custom_funs[func_str]
