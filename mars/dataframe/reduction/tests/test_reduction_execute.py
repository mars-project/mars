...
class TestCorr(TestBase):
    ...
    def testDataFrameCorrWith(self):
        ...
        df = from_pandas_df(raw_df)
        df2 = from_pandas_df(raw_df2)
        ...
        df = from_pandas_df(raw_df, chunk_size=4)
        df2 = from_pandas_df(raw_df2, chunk_size=6)
        s = from_pandas_series(raw_s, chunk_size=5)
        s2 = from_pandas_series(raw_s2, chunk_size=5)
    ...
    def testSeriesCorr(self):
        ...
        s = from_pandas_series(raw, chunk_size=6)
        s2 = from_pandas_series(raw2, chunk_size=4)
