...
def explode_series(series):
    ...
    # TODO: remove it when we drop support for python2.7
    ...
    value_lists = series.to_list()
    values = itertools.chain(*[sublist if isinstance(sublist, list) else [sublist] for sublist in value_lists])
...
class DataFrameNunique(DataFrameReductionOperand, DataFrameReductionMixin):
    ...
    def _execute_combine(cls, ctx, op):
        ...
        if isinstance(in_data, xdf.Series):
            unique_values = explode_series(in_data).drop_duplicates()
            ctx[op.outputs[0].key] = xdf.Series(unique_values, name=in_data.name)
        else:
            if op.axis == 0:
                df = xdf.DataFrame(OrderedDict((d, [explode_series(v).drop_duplicates().to_list()])
                                               for d, v in in_data.iteritems()))
            else:
                df = xdf.DataFrame(columns=[0])
                for d, v in in_data.iterrows():
                    df.loc[d] = [explode_series(v).drop_duplicates().to_list()]
            ctx[op.outputs[0].key] = df
    ...
    def _execute_agg(cls, ctx, op):
        ...
        in_data = ctx[op.inputs[0].key]
        if isinstance(in_data, xdf.Series):
            ctx[op.outputs[0].key] = explode_series(in_data).nunique(dropna=op.dropna)
        else:
            if op.axis == 0:
                ctx[op.outputs[0].key] = xdf.Series(OrderedDict((d, explode_series(v).nunique(dropna=op.dropna))
                                                    for d, v in in_data.iteritems()))
            else:
                ctx[op.outputs[0].key] = xdf.Series(OrderedDict((d, explode_series(v).nunique(dropna=op.dropna))
                                                    for d, v in in_data.iterrows()))
