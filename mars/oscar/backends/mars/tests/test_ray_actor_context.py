...
from . import test_mars_actor_context
...
def actor_pool_context():
    ...
    for addr in [pg_bundle_to_address(pg_name, 0, process_index=i) for i in range(n_process)]:
        try:
            ray.kill(ray.get_actor(addr))
        except Exception:
            pass
    from ..communication.ray import RayServer
    RayServer._address_to_instances = dict()
