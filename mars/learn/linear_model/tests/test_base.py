...
from mars.learn.linear_model._base import _deprecate_normalize
...
def test_deprecate_normalize(normalize, default):
    ...
    if n_warnings:
        assert all([warning in str(record[0].message)
...
def test_preprocess_data_weighted(is_sparse):
    ...
    expected_X_scale = np.sqrt(X_sample_weight_var) \
    ...
    if is_sparse:
        ...
        assert_array_almost_equal(
            scaler.transform(X).toarray()[:, :2] / np.sqrt(
                sample_weight.sum()),
            Xt.toarray()[:, :2],
        )
