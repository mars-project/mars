...
from .._logistic import (_check_solver,
                         _check_multi_class,
                         LogisticRegression)
...
def test_check_solver(setup):
    ...
    error_msg = (re.escape("Logistic Regression supports only solvers in %s, "
...
def test_check_multi_class(setup):
    ...
    error_msg = (re.escape("Solver %s does not support "
    ...
    error_msg = (re.escape("multi_class should be 'multinomial', "
                           "'ovr' or 'auto'. Got %s."
                           % invalid_multi_class))
    with pytest.raises(ValueError, match=error_msg):
...
def test_invalid_C(setup):
    invalid_C = -1
    error_msg = (re.escape("Penalty term must be positive; got (C=%r)"
                           % invalid_C))
...
def test_invalid_max_iter(setup):
    invalid_max_iter = -1
    error_msg = (re.escape("Maximum number of iteration must be positive;"
