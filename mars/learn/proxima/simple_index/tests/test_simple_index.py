...
def build_and_query(doc, query, topk, column_number, row_number,
...
def consistency_checking(doc, query, dimension, topk, measure_name,
    ...
    # mars_data
    pk_m, distance_m = build_and_query(doc, query, dimension=dimension, topk=topk, threads=threads,
...
def test_build_and_search_index(setup):
    ...
    for i, index_builder in enumerate(index_builder_lists):
        for measure_name in measure_name_lists:
            consistency_checking(doc=doc, query=query, dimension=dimension, topk=topk, threads=threads,
    ...
    for i, index_builder in enumerate(index_builder_lists):
        for measure_name in measure_name_lists:
            consistency_checking(doc, query, dimension=dimension, topk=topk, threads=threads,
                                 measure_name=measure_name, column_number=column_number, row_number=row_number,
                                 index_builder=index_builder, builder_params=builder_params_lists[i],
                                 index_converter=index_converter_lists[i],
                                 index_converter_params=index_converter_params,
...
def test_recall(setup):
    ...
    # proxima_data
    pk_p, distance_p = build_and_query(doc, query, dimension=dimension, topk=topk,
                                       threads=1, column_number=2, row_number=3)
    assert isinstance(recall(doc=doc, query=query, topk=topk, sample_count=sample_count,
