...
class TaskWebAPIHandler(MarsServiceWebAPIHandler):
    ...
    async def submit_tileable_graph(self, session_id: str):
        task_name = self.get_argument('task_name', None) or None
        fuse_enabled = bool(int(self.get_argument('fuse', '1')))
        graph = deserialize_serializable(self.get_argument('graph'))
        extra_config_ser = self.get_argument('extra_config', None)
        if extra_config_ser is not None:
            extra_config = deserialize_serializable(extra_config_ser)
        else:
            extra_config = None
    ...
    async def wait_task(self, session_id: str, task_id: str):
        ...
        oscar_api = await self._get_oscar_task_api(session_id)
        await oscar_api.wait_task(task_id, timeout)
...
class WebTaskAPI(AbstractTaskAPI, MarsWebAPIClientMixin):
    ...
    async def submit_tileable_graph(self,
        ...
        path = f'{self._session_id}/api/session/{self._session_id}/task'
        ...
        body = self._make_multipart_form({
            ...
            'fuse': '1' if fuse_enabled else '0',
            'graph': serialize_serializable(graph),
            'extra_config': extra_config_ser
        })
        res = await self._request_url(path, method='post', body=body)
        return res.body

    async def get_fetch_tileables(self, task_id: str) -> List[Tileable]:
        path = f'{self._session_id}/api/session/{self._session_id}/task/{task_id}' \
