...
web_handlers = {
    '/api/service/session/(.*)': SessionWebHandler,
}
...
class SessionWebAPI(ServiceWebAPIBase):
    ...
    async def create(cls, address: str, **kwargs):
        ...
        resp = await http_client.fetch(f'{get_web_address()}/api/service/session/create',
                                       method="POST", body=serialize((address, kwargs)))
        api_id = deserialize(resp.body)
        return SessionWebAPI(http_client, 'session', api_id)
