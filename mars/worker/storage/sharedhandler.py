...
class SharedStorageIO(BytesStorageIO):
    ...
    async def __init__(self, session_id, data_key, mode='w', shared_store=None,
        ...
        if self.is_writable:
            self._shared_buf = await shared_store.create(session_id, data_key, nbytes)
            if packed:
            ...
            self._shared_buf = await shared_store.get_buffer(session_id, data_key)
            if packed:
    ...
    async def close(self, finished=True):
        ...
        if self.is_writable and self._shared_buf is not None:
            await self._shared_store.seal(self._session_id, self._data_key)
            if finished:
                if self._auto_register:
                    await self._holder_ref.put_objects_by_keys(
                        self._session_id, [self._data_key], pin_token=self._pin_token)
            else:
                await self._shared_store.delete(self._session_id, self._data_key)
...
class SharedStorageHandler(StorageHandler, BytesStorageMixin, ObjectStorageMixin,
    ...
    async def create_bytes_reader(self, session_id, data_key, packed=False, packed_compression=None,
                                  _promise=False):
        return await SharedStorageIO(session_id, data_key, 'r', self._shared_store, packed=packed,
    ...
    async def create_bytes_writer(self, session_id, data_key, total_bytes, packed=False,
                                  ...
                                  _promise=False):
        return await SharedStorageIO(session_id, data_key, 'w', self._shared_store,
    ...
    async def put_objects(self, session_id, data_keys, objs, sizes=None, serialize=False,
        ...
        try:
            for key, obj in zip(data_keys, objs):
                ...
                try:
                    obj_refs.append(await self._shared_store.put(session_id, key, obj))
                    ...
                    if self.location in (await self.storage_ctx.get_data_locations(session_id, [key]))[0]:
                        succ_keys.append(key)
                        succ_shapes.append(shape)
                    else:
                        raise
            await self._holder_ref.put_objects_by_keys(
                session_id, succ_keys, shapes=succ_shapes, pin_token=pin_token)
            if affected_keys:
    ...
    async def load_from_bytes_io(self, session_id, data_keys, src_handler, pin_token=None):
        ...
        async def _handle_writer_create_fail(key, reader, exc_info):
            nonlocal is_success
            await reader.close()
            failed_keys.add(key)
            exc = exc_info[1]
        ...
        async def _on_file_close(key, _reader, writer, finished):
            if finished:
                if is_success:
        ...
        async def _finalize_load(*exc_info):
            try:
                success_keys = [k for k in data_keys if k not in failed_keys]
                if success_keys:
                    await self._holder_ref.put_objects_by_keys(session_id, success_keys, pin_token=pin_token)
                if exc_info:
        ...
        async def _copy_data(k, reader):
            ...
            return p.then(lambda writer: self._copy_bytes_data(reader, writer, on_close),
                          lambda *exc: handle_worker_create_fail(reader, exc))

        async def _load_single_key(k):
    ...
    async def delete(self, session_id, data_keys, _tell=False):
        await self._holder_ref.delete_objects(session_id, data_keys, _tell=_tell)
    ...
    async def lift_data_keys(self, session_id, data_keys, _tell=True):
        await self._holder_ref.lift_data_keys(session_id, data_keys, _tell=_tell)

    async def pin_data_keys(self, session_id, data_keys, token):
        return await self._holder_ref.pin_data_keys(session_id, data_keys, token)

    async def unpin_data_keys(self, session_id, data_keys, token, _tell=False):
        return await self._holder_ref.unpin_data_keys(session_id, data_keys, token, _tell=_tell)
