...
class ObjectHolderActor(WorkerActor):
    ...
    def spill_size(self, size, multiplier=1, callback=None):
        if not self._spill_devices:
            raise NotImplementedError('Spill device not specified for %s' % type(self).__name__)
    ...
    def _internal_put_object(self, session_id, data_key, obj, size):
        ...
        logger.debug('Chunk %s registered in %s. total_hold=%d', data_key, self.uid, self._total_hold)
        self.update_cache_status()
    ...
    @log_unhandled
    def delete_objects(self, session_id, chunk_keys):
        for c in chunk_keys:
            self.delete_object(session_id, c)
    ...
    def unpin_data_keys(self, session_id, data_keys, token):
        ...
        for k in data_keys:
            session_k = (session_id, k)
            try:
                self._pinned_counter[session_k].difference_update((token,))
...
class InProcHolderActor(ObjectHolderActor):
    ...
    def gen_uid(cls, proc_id):
        return 'w:%d:%s' % (proc_id, cls.__name__)
