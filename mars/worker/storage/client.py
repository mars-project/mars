...
class StorageClient:
    ...
    async def get_storage_handler(self, location):
        try:
            return self._storage_handlers[location]
        except KeyError:
            handler = self._storage_handlers[location] = \
                await get_storage_handler_cls(location[1])(self, proc_id=location[0])
            return handler
    ...
    async def _do_with_spill(self, action, data_keys, total_bytes, device_order,
                             device_pos=0, spill_multiplier=1.0, ensure=True, data_cleaner=None):
        async def _handle_err(*exc_info):
            if issubclass(exc_info[0], StorageFull):
                ...
                if device_pos < len(device_order) - 1:
                    return await self._do_with_spill(
                    ...
                    return (await handler.spill_size(req_bytes, spill_multiplier)) \
        ...
        handler = await self.get_storage_handler(cur_device)
        return (await action(handler, data_keys)).catch(_handle_err)

    async def create_reader(self, session_id, data_key, source_devices, packed=False,
        ...
        if _promise:
            return (await self.copy_to(session_id, [data_key], source_devices)) \
    ...
    async def create_writer(self, session_id, data_key, total_bytes, device_order, packed=False,
                            packed_compression=None, pin_token=None, _promise=True):
        device_order = self._normalize_devices(device_order)

        async def _action(handler, _keys, _promise=True):
            ...
            return await handler.create_bytes_writer(
                session_id, data_key, total_bytes, packed=packed, packed_compression=packed_compression,
                pin_token=pin_token, _promise=_promise)

        if _promise:
            return await self._do_with_spill(_action, [data_key], total_bytes, device_order)
        else:
            exc = ValueError('Missing device type')
            for device_type in device_order:
                try:
                    handler = await self.get_storage_handler(device_type)
                    return await _action(handler, [data_key], _promise=False)
                except StorageFull as ex:
                    exc = ex
            raise exc

    async def get_object(self, session_id, data_key, source_devices, serialize=False, _promise=True):
        ...
        if _promise:
            return (await self.get_objects(
                session_id, [data_key], source_devices, serialize=serialize, _promise=True)).then(_extract_obj)
        else:
            return _extract_obj(await self.get_objects(
                session_id, [data_key], source_devices, serialize=serialize, _promise=False))

    async def get_objects(self, session_id, data_keys, source_devices, serialize=False, _promise=True):
        source_devices = self._normalize_devices(source_devices)
        stored_dev_lists = await self._manager_ref.get_data_locations(session_id, data_keys)
        ...
        if not _promise:
            try:
                ...
                for stored_dev, keys in dev_to_keys.items():
                    handler = await self.get_storage_handler(stored_dev)
                    data_dict.update(zip(keys, await handler.get_objects(
            ...
            try:
                ...
                for stored_dev, keys in dev_to_keys.items():
                    ...
                    if stored_dev in source_devices:
                        promises.append(
                            (await handler.get_objects(session_id, keys, serialize=serialize, _promise=True))
                                .then(updater, _handle_key_error)
                        )
                    else:
                        promises.append(
                            (await self.copy_to(session_id, keys, source_devices)).then(loc_getter)
                                .then(updater, _handle_key_error)
                        )
    ...
    async def put_objects(self, session_id, data_keys, objs, device_order, sizes=None, serialize=False):
        device_order = self._normalize_devices(device_order)
        if sizes:
        ...
        async def _action(h, keys):
            objects = [data_dict[k] for k in keys]
            data_sizes = [sizes_dict[k] for k in keys]
            try:
                return await h.put_objects(session_id, keys, objects, sizes=data_sizes, serialize=serialize,
                                           _promise=True)
            finally:
                objects[:] = []
                del objects
        ...
        return (await self._do_with_spill(_action, data_keys, sum(sizes_dict.values()), device_order,
                                          data_cleaner=_clean_succeeded)) \
            .then(lambda *_: _clean_all(), _clean_all)

    async def copy_to(self, session_id, data_keys, device_order, ensure=True, pin_token=None):
        device_order = self._normalize_devices(device_order)
        existing_devs = await self._manager_ref.get_data_locations(session_id, data_keys)
        data_sizes = await self._manager_ref.get_data_sizes(session_id, data_keys)
        ...
        for target, data_keys in lift_reqs.items():
            handler = await self.get_storage_handler(target)
            if getattr(handler, '_spillable', False):
                await handler.lift_data_keys(session_id, data_keys)
        ...
        async def _handle_exc(keys, *exc):
            existing = await self._manager_ref.get_data_locations(session_id, keys)
        ...
        for d in device_to_keys.keys():
            action = functools.partial(_action, await self.get_storage_handler(d))
            keys = device_to_keys[d]
            total_size = device_total_size[d]
            promises.append(
                (await self._do_with_spill(action, keys, total_size, device_order, ensure=ensure))
                    .catch(functools.partial(_handle_exc, keys))
            )
        return promise.all_(promises)

    async def delete(self, session_id, data_keys, devices=None, _tell=False):
        if not devices:
            devices = functools.reduce(
                operator.ior, await self._manager_ref.get_data_locations(session_id, data_keys), set())
        else:
            devices = self._normalize_devices(devices)

        devices = self._normalize_devices(devices)
        for dev_type in devices:
            handler = await self.get_storage_handler(dev_type)
            await handler.delete(session_id, data_keys, _tell=_tell)

    async def filter_exist_keys(self, session_id, data_keys, devices=None):
        devices = self._normalize_devices(devices or DataStorageDevice.__members__.values())
        return await self.manager_ref.filter_exist_keys(session_id, data_keys, devices)

    async def pin_data_keys(self, session_id, data_keys, token, devices=None):
        if not devices:
            devices = functools.reduce(
                operator.ior, await self._manager_ref.get_data_locations(session_id, data_keys), set())
        else:
        ...
        pinned = set()
        for dev in devices:
            handler = await self.get_storage_handler(dev)
            if not getattr(handler, '_spillable', False):
                continue
            keys = await handler.pin_data_keys(session_id, data_keys, token)
            pinned.update(keys)
        return list(pinned)

    async def unpin_data_keys(self, session_id, data_keys, token, devices=None):
        if not devices:
            devices = functools.reduce(
                operator.ior, await self._manager_ref.get_data_locations(session_id, data_keys), set())
        else:
        ...
        for dev in devices:
            ...
            if not getattr(handler, '_spillable', False):
                continue
            await handler.unpin_data_keys(session_id, data_keys, token)

    async def spill_size(self, data_size, devices):
        promises = []
        devices = self._normalize_devices(devices)
        for dev in devices:
            handler = await self.get_storage_handler(dev)
            if not getattr(handler, '_spillable', False):
                continue
            promises.append(await handler.spill_size(data_size))
        return promise.all_(promises)
