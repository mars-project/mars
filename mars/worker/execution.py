        ...
        def _finish_fetch(*_):
            ...
            if self._chunk_holder_ref.is_stored(chunk_key):
                self._mem_quota_ref.release_quota(
                    build_load_key(graph_key, chunk_key), _tell=True, _wait=False)
        ...
        if all(k in save_sizes for k in graph_record.data_targets):
            ...
            try:
                ...
                self.ref().execute_graph(
                    ...
                    graph_record.data_metas, send_addresses=send_addresses,
                    callback=graph_record.finish_callbacks, _tell=True, _delay=retry_delay)
                return
