...
from ..datasource import tensor as astensor, linspace, array
from ..arithmetic import log2, absolute, subtract
from ..arithmetic.utils import tree_add
from ..reduction import std, mean
from ..merge import stack
from ..utils import is_asc_sorted, recursive_tile
from ..array_utils import as_same_device, device
from ..base.where import where
from ..base.diff import diff
from ..base.ndim import ndim
...
class HistBinScottSelector(HistBinSelector):
    ...
    def __call__(self):
        return (24.0 * np.pi**0.5 / self._x.size)**(1.0 / 3.0) * std(self._x)
...
class HistBinStoneSelector(HistBinSelector):
    ...
    def __call__(self):
        ...
        nbins = stack(candidates).argmin() + 1
        return ptp_x / nbins
...
class HistBinDoaneSelector(HistBinSelector):
    ...
    def __call__(self):
        ...
        sigma = std(x)
        g1 = mean(((x - mean(x)) / sigma)**3)
        ret = _ptp(self._raw_range) / (1.0 + np.log2(x.size) +
                                      log2(1.0 + absolute(g1) / sg1))
        return where(sigma > 0.0, ret, 0.0)
...
class HistBinFdSelector(HistBinSelector):
    ...
    def __call__(self):
        iqr = subtract(*percentile(self._x, [75, 25]))
...
class HistBinAutoSelector(HistBinSelector):
    ...
    def __init__(self, histogram_bin_edges_op, x, range, raw_range):
        super(HistBinAutoSelector, self).__init__(
    ...
    def get_result(self):
        fd_bw = super(HistBinAutoSelector, self).get_result()
...
def _get_bin_edges(op, a, bins, range, weights):
    ...
    if isinstance(bins, str):
        ...
        assert ndim(bins) == 1 and not isinstance(bins, TENSOR_TYPE)
    ...
    if n_equal_bins is not None:
        ...
        return astensor(bin_edges), None
...
class TensorHistogramBinEdges(TensorOperand, TensorOperandMixin):
    ...
    def __call__(self, a, bins, range, weights):
        ...
        if a.size > 0 and \
                (isinstance(bins, str) or ndim(bins) == 0) and not range:
