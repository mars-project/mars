...
from ...compat import six, OrderedDict, Enum
...
class IndexInfo(object):
    def __init__(self, index_type, input_axis, output_axis,
                 raw_index, handler):
...
class FancyIndexInfo(IndexInfo):
    def __init__(self, index_type, input_axis, output_axis, raw_index, handler):
        super(FancyIndexInfo, self).__init__(index_type, input_axis, output_axis,
                                             raw_index, handler)
...
class IndexHandlerContext(object):
    ...
    def create_chunk(self, chunk_index, chunk_index_info):
...
class IndexHandler(six.with_metaclass(ABCMeta)):
    _instance = None
    ...
    @abstractmethod
    def parse(self, raw_index, context):
        pass

    def preprocess(self, index_info, context):
        pass
    ...
    def postprocess(self, index_info, context):
        pass
    ...
    def process(self, index_info, context):
        ...
        for chunk_index, chunk_index_info in index_to_info.items():
            ...
            if i not in effected_i_to_slice:
                ...
                chunk_index_info.set(next(other_index_to_iter[other_index]),
                                     slc, output_shape)
    ...
    def process(self, index_info, context):
        ...
        for chunk_index, chunk_index_info in index_to_info.items():
            ...
            if i not in effected_i_to_slice:
                ...
                slc = effected_i_to_slice[i]
                chunk_index_info.set(None, slc, None)
...
class _FancyIndexHandler(IndexHandler):
    ...
    @classmethod
    def is_first(cls, index_info, context):
...
class NDArrayFancyIndexHandler(_FancyIndexHandler):
    ...
    def preprocess(self, index_info, context):
        ...
        super(NDArrayFancyIndexHandler, self).preprocess(index_info, context)
        check_chunks_unknown_shape([context.tileable], TilesError)
    ...
    def process(self, index_info, context):
        ...
        for chunk_index, chunk_index_info in chunk_index_to_info.items():
            ...
            chunk_index_info.set(
                output_axis_index, fancy_index_array, output_axis_shape)
    ...
    def postprocess(self, index_info, context):
        ...
        for chunk_index in itertools.product(
            ...
            reorder_index = concat_chunk.index[:to_concat_axis] + (0,) * len(fancy_index_shape) + \
...
class TensorFancyIndexHandler(_FancyIndexHandler):
    ...
    def preprocess(self, index_info, context):
        ...
        super(TensorFancyIndexHandler, self).preprocess(index_info, context)
    ...
    @classmethod
    def _shuffle_fancy_indexes(cls, concat_fancy_index, context, index_info, axes):
    ...
    def process(self, index_info, context):
        ...
        for chunk_index, chunk_index_info in context.chunk_index_to_info.items():
            ...
            chunk_index_info.set(output_axis_index, fancy_index_chunk, output_axis_shape)
...
class IndexesHandler(object):
    ...
    def handle(self, op, return_context=False):
        indexes = op.indexes
        # create context
        context = IndexHandlerContext(op)
    ...
    @classmethod
    def _preprocess(cls, context, index_infos):
        # preprocess
...
class NDArrayIndexesHandler(IndexesHandler):
    ...
    def __init__(self):
        super(NDArrayIndexesHandler, self).__init__()
    ...
    @classmethod
    def aggregate_result(cls, context, chunk_index_result):
...
class TensorIndexesHandler(IndexesHandler):
    def __init__(self):
        super(TensorIndexesHandler, self).__init__()
        self.register(NewaxisIndexHandler,
                      SliceIndexHandler,
                      IntegralIndexHandler,
                      NDArrayBoolIndexHandler,
