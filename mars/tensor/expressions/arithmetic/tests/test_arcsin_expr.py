...
from mars.tensor.expressions.arithmetic import arcsin, TensorArcsin
...
class Test(TestBase):
    ...
    def testTensorSerialize(self):
        t = arcsin(tensor(self.raw_data, chunk_size=(3, 4)))

        serials = self._pb_serial(t)
        t2 = self._pb_deserial(serials)[t.data]

        self.assertIsInstance(t2.op, TensorArcsin)
    ...
    def testChunkSerialize(self):
        ...
        self.assertIsInstance(c2.op, TensorArcsin)
    ...
    def testDenseExpr(self):
        ...
        # test arcsin with out, wrong type of out
        with self.assertRaises(TypeError):
            arcsin(t, out=self.raw_data)
        with self.assertRaises(TypeError):
            arcsin(t, out=1)

        # test arcsin with out, wrong shape of out
        with self.assertRaises(ValueError):
            arcsin(t, out=t[:, 0])

        # test arcsin with out, wrong dtype of out
        with self.assertRaises(TypeError):
            arcsin(t, out=t.astype('i8'))
    ...
    def testSparseExpr(self):
        ...
        # test arcsin with out, wrong type of out
        with self.assertRaises(TypeError):
            arcsin(t, out=self.raw_sparse_data)
        with self.assertRaises(TypeError):
            arcsin(t, out=1)
        ...
        # test arcsin with out, sparse output to dense
        with self.assertRaises(ValueError):
            arcsin(t, out=t.todense())
