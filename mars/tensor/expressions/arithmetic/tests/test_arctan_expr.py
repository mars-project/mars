...
from mars.tensor.expressions.arithmetic import arctan, TensorArctan
...
class Test(TestBase):
    ...
    def testTensorSerialize(self):
        t = arctan(tensor(self.raw_data, chunk_size=(3, 4)))

        serials = self._pb_serial(t)
        t2 = self._pb_deserial(serials)[t.data]

        self.assertIsInstance(t2.op, TensorArctan)
    ...
    def testChunkSerialize(self):
        ...
        self.assertIsInstance(c2.op, TensorArctan)
    ...
    def testDenseExpr(self):
        ...
        # test arctan with out, wrong type of out
        with self.assertRaises(TypeError):
            arctan(t, out=self.raw_data)
        with self.assertRaises(TypeError):
            arctan(t, out=1)

        # test arctan with out, wrong shape of out
        with self.assertRaises(ValueError):
            arctan(t, out=t[:, 0])

        # test arctan with out, wrong dtype of out
        with self.assertRaises(TypeError):
            arctan(t, out=t.astype('i8'))
    ...
    def testSparseExpr(self):
        ...
        # test arctan with out, wrong type of out
        with self.assertRaises(TypeError):
            arctan(t, out=self.raw_sparse_data)
        with self.assertRaises(TypeError):
            arctan(t, out=1)
        ...
        # test arctan with out, sparse output to dense
        with self.assertRaises(ValueError):
            arctan(t, out=t.todense())
