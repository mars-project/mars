...
from mars.tensor.expressions.arithmetic import arccosh, TensorArccosh
...
class Test(TestBase):
    ...
    def testTensorSerialize(self):
        t = arccosh(tensor(self.raw_data, chunk_size=(3, 4)))

        serials = self._pb_serial(t)
        t2 = self._pb_deserial(serials)[t.data]

        self.assertIsInstance(t2.op, TensorArccosh)
    ...
    def testChunkSerialize(self):
        ...
        self.assertIsInstance(c2.op, TensorArccosh)
    ...
    def testDenseExpr(self):
        ...
        # test arccosh with out, wrong type of out
        with self.assertRaises(TypeError):
            arccosh(t, out=self.raw_data)
        with self.assertRaises(TypeError):
            arccosh(t, out=1)

        # test arccosh with out, wrong shape of out
        with self.assertRaises(ValueError):
            arccosh(t, out=t[:, 0])

        # test arccosh with out, wrong dtype of out
        with self.assertRaises(TypeError):
            arccosh(t, out=t.astype('i8'))

        # test out
        t1 = tensor(self.raw_data, chunk_size=(3, 4))
        t1_data = t1.data
        t2 = t[0, :].astype('f4')
        t3 = arccosh(t2, out=t1)
        t3.tiles()
    ...
    def testSparseExpr(self):
        ...
        # test arccosh with out, wrong type of out
        with self.assertRaises(TypeError):
            arccosh(t, out=self.raw_sparse_data)
        with self.assertRaises(TypeError):
            arccosh(t, out=1)
