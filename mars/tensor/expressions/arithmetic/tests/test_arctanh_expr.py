...
from mars.tensor.expressions.arithmetic import arctanh, TensorArctanh
...
class Test(TestBase):
    ...
    def testTensorSerialize(self):
        t = arctanh(tensor(self.raw_data, chunk_size=(3, 4)))

        serials = self._pb_serial(t)
        t2 = self._pb_deserial(serials)[t.data]

        self.assertIsInstance(t2.op, TensorArctanh)
    ...
    def testChunkSerialize(self):
        ...
        self.assertIsInstance(c2.op, TensorArctanh)
    ...
    def testDenseExpr(self):
        ...
        # test arctanh with out, wrong type of out
        with self.assertRaises(TypeError):
            arctanh(t, out=self.raw_data)
        with self.assertRaises(TypeError):
            arctanh(t, out=1)

        # test arctanh with out, wrong shape of out
        with self.assertRaises(ValueError):
            arctanh(t, out=t[:, 0])

        # test arctanh with out, wrong dtype of out
        with self.assertRaises(TypeError):
            arctanh(t, out=t.astype('i8'))

        # test out
        t1 = tensor(self.raw_data, chunk_size=(3, 4))
        t1_data = t1.data
        t2 = t[0, :].astype('f4')
        t3 = arctanh(t2, out=t1)
        t3.tiles()
    ...
    def testSparseExpr(self):
        ...
        # test arctanh with out, wrong type of out
        with self.assertRaises(TypeError):
            arctanh(t, out=self.raw_sparse_data)
        with self.assertRaises(TypeError):
            arctanh(t, out=1)
        ...
        # test arctanh with out, sparse output to dense
        with self.assertRaises(ValueError):
            arctanh(t, out=t.todense())
