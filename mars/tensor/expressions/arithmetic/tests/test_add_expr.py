...
from mars.tensor.expressions.arithmetic import add, TensorAdd, TensorAddConstant
...
class Test(TestBase):
    ...
    def testTensorSerialize(self):
        ...
        self.assertIsInstance(t2.op, TensorAdd)  # not constant
        self.assertIsInstance(t2.op.out.op, t_op_type)
        self.assertEqual(t.issparse(), t2.issparse())

        # test add with out and where
        t = tensor(self.raw_data1, chunk_size=(3, 4))
        w = tensor(self.raw_data1 < 0.5, chunk_size=(3, 4))
        t_op_type = type(t.op)
        w_op_type = type(w.op)
        t = add(t, 1, out=t, where=w)
    ...
    def testChunkSerialize(self):
        ...
        self.assertIsInstance(c2.op, TensorAdd)  # not constant
