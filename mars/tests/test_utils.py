...
def test_log_exception_wrapper():
    ...
    try:
        p = multiprocessing.Process(target=_log_exception, args=(log_file_name,))
        p.start()
        while p.is_alive():
            time.sleep(0.1)
        with open(log_file_name, "r") as f:
            ex_log = f.read()
            assert ex_log.count("raise_ex1") == 3, ex_log
            assert ex_log.count("exception func1") == 1, ex_log
            assert ex_log.count("raise_ex2") == 3, ex_log
            assert ex_log.count("exception func2") == 1, ex_log
    finally:
        if os.path.exists(log_file_name):
            os.remove(log_file_name)


def _log_exception(log_file_name):
    from ..utils import logger, log_exception_wrapper, sync_to_async
    ...
    async def main():
        for func in [
            log_exception_wrapper(raise_ex1, "%s failed", "exception func1"),
            log_exception_wrapper(raise_ex1, "%s failed"),
            log_exception_wrapper(raise_ex1),
            log_exception_wrapper(raise_ex2, "%s failed", "exception func2"),
            log_exception_wrapper(raise_ex2, "%s failed"),
            log_exception_wrapper(raise_ex2),
            log_exception_wrapper(normal),
        ]:
