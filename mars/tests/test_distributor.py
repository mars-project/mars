...
class Test(unittest.TestCase):
    def testDistributor(self):
        ...
        self.assertEqual(distributor.distribute(uid),
                         int(hashlib.md5(to_binary(uid)).hexdigest(), 16) % distributor.n_process)

        uid = 'w:h1:Actor'
        self.assertEqual(distributor.distribute(uid),
                         1 + int(hashlib.md5(to_binary(uid)).hexdigest(), 16) % (distributor.n_process - 1))
        ...
        self.assertEqual(new_uid,
                         'w:%d:Actor' % (1 + int(hashlib.md5(to_binary(ref_uid)).hexdigest(), 16)
